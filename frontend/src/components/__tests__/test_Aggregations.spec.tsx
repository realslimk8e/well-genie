...
const getAggregatedRows = (title: string) => {
  ...
  const container = heading.closest('.card-body') ?? heading.parentElement;
...
describe('Aggregated summaries', () => {
  ...
  test('SummaryFilters updates table when date range and category toggles change', async () => {
    ...
    expect(updatedCells.some((cell) => cell.textContent === 'steps')).toBe(
      false,
    );
  });
  ...
  test('SummaryFilters allows sorting and show/hide columns', async () => {
    ...
    const table = screen.getByRole('table');
    const tableCategoryButton = within(table).getByRole('button', {
      name: /category/i,
    });
    ...
    expect(
      within(table).queryByRole('columnheader', { name: /count/i }),
    ).toBeNull();
  });
  ...
  test('Panels show standard aggregations and indicate aggregation type', () => {
    ...
    expect(screen.getByTestId('sleep-aggregations')).toHaveTextContent(
      /sum 28\.0/i,
    );
    expect(screen.getByTestId('sleep-aggregations')).toHaveTextContent(
      /avg 7\.0/i,
    ...
    expect(screen.getByTestId('diet-aggregations')).toHaveTextContent(
      /sum 8200/i,
    );
    expect(screen.getByTestId('diet-aggregations')).toHaveTextContent(
      /avg 2050\.0/i,
    ...
    expect(screen.getByTestId('exercise-aggregations')).toHaveTextContent(
      /sum 150/i,
    );
    expect(screen.getByTestId('exercise-aggregations')).toHaveTextContent(
      /avg 37\.5/i,
  ...
  test('SummaryFilters allows deleting data for a category and date range', async () => {
    ...
    const sleepRow = rows.find((r) =>
      (r.textContent || '').toLowerCase().includes('sleep'),
    );
    expect(sleepRow?.textContent || '').toContain('0');
    expect(await screen.findByRole('status')).toHaveTextContent(
      /deleted 3 sleep record/i,
    );
  });
});
